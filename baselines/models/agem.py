...
class AGem(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        self.grad_dims = []
        self.args.num_tasks = args.num_tasks
        for param in self.parameters():
            self.grad_dims.append(param.data.numel())
        self.grad_xy = torch.Tensor(np.sum(self.grad_dims)).to(self.device)
        self.grad_er = torch.Tensor(np.sum(self.grad_dims)).to(self.device)

    def end_task(self, inputs, labels):
        samples_per_task = self.args.buffer_size // self.args.num_tasks #dataset.N_TASKS
        #loader = dataset.train_loader
        #cur_y, cur_x = next(iter(loader))[1:]
        self.buffer.add_data(
            examples=inputs.to(self.device),
