...
class AsymmetricQuantFunction(Function):
    ...
    def forward(ctx, x, k, x_min, x_max):
        ...
        if signed:
            max_clamp = n - 1
            new_quant_x = torch.clamp(new_quant_x, -n, max_clamp)
        else:
            max_clamp = 2 * n - 1
            new_quant_x = torch.clamp(new_quant_x, 0, max_clamp)
        ctx.save_for_backward(max_clamp)
    ...
    def backward(ctx, grad_output):
        ...
        max_clamp = ctx.saved_tensors
        i = (input > max_clamp).float()
