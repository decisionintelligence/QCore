...
if __name__ == "__main__":
    ...
    parser.add_argument('--set', type=str, default='Adiac')
    ...
    for d, _, fs in os.walk(args.input_dir):
        for f in fs:
            ...
            if f.endswith(args.set + '_stats_dict.pkl') and check_filename(
                ...
                ordered_examples, ordered_values = sort_examples_by_forgetting([unlearned_per_presentation], [first_learned], args.epochs)

                #DF = pd.DataFrame(ordered_examples)
                #DF.to_csv(f+"_examples.csv")
                #DF = pd.DataFrame(ordered_values)
                #DF.to_csv(f+"_values.csv")
    ...
    if len(unlearned_per_presentation_all) == 0:
        ...
        df4 = df3.sample(n=int(args.core_size*len(df1)), weights='values', random_state=1).sort_values('values')
        df5 = pd.DataFrame(df4.index)
        df5.to_csv(args.set + '_index.csv', index=False,header=None)
