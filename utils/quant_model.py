...
class Conv1dQuantizer(nn.Module):
    ...
    def set_flipping(self,args):
        ...
        self.model_1 = Flipping(9,args).to(args.device)
        if args.quant_flipping == True:
            self.model_1 = quantize_model(self.model_1)
        self.model_1.load_state_dict(torch.load("./models/" + args.data_source + "_" + args.dataset + "_Quant_" + str(args.bits) + "_Kernel_1.pth"))
        self.model_1.eval()

        self.model_2 = Flipping(19,args).to(args.device)
        if args.quant_flipping == True:
            self.model_2 = quantize_model(self.model_2)
        self.model_2.load_state_dict(torch.load("./models/" + args.data_source + "_" + args.dataset + "_Quant_" + str(args.bits) + "_Kernel_2.pth"))
        self.model_2.eval()

        self.model_3 = Flipping(39,args).to(args.device)
        if args.quant_flipping == True:
            self.model_3 = quantize_model(self.model_3)
        self.model_3.load_state_dict(torch.load("./models/" + args.data_source + "_" + args.dataset + "_Quant_" + str(args.bits) + "_Kernel_3.pth"))
        self.model_3.eval()
